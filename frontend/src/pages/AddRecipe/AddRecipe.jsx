...
import { FiChevronLeft, FiPlusCircle, FiFileText } from "react-icons/fi";
...
export default function AddRecipe() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data && response.data.success) {
                const created = response.data.recipe;
                setToastMsg(`Successfully added "${created.title}"!`);
                setIsToastOpen(true);
                setTimeout(() => {
            ...
            const errMsg = error.response?.data?.message || "Failed to publish recipe. Check all fields.";
            alert(errMsg);
        } finally {
    ...
    return (
        <Container className="py-6 max-w-2xl text-left">
            <div className="mb-6">
                <Link to="/recipes" className="flex items-center gap-1 text-sm font-semibold text-text/60 hover:text-text-h transition-colors">
                    <FiChevronLeft /> Back to Catalogue
                </Link>
            </div>
            ...
            <div className="border border-border rounded-card p-6 sm:p-8 bg-card shadow-xs">
                <header className="mb-6 pb-4 border-b border-border flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-extrabold text-text-h font-heading m-0 mb-0.5">Create New Recipe</h1>
                        <p className="text-text/60 text-xs m-0">Add a custom culinary dish to your kitchen notebook.</p>
                    </div>
                </header>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                    ...
                    <div className="border-t border-border pt-6 mt-2 flex justify-end gap-3">
                        ...
                        >
                            <FiPlusCircle size={16} /> {isSubmitting ? "Publishing..." : "Publish Recipe"}
                        </Button>
                    </div>
                </form>
            </div>
